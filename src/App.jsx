    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {tab==="kpi"&&<><Sel value={year} onChange={e=>sYear(+e.target.value)} style={{width:88}}>{[2026,2027,2028].map(y=><option key={y}>{y}</option>)}</Sel><Inp type="number" value={annual} onChange={e=>sAnn(+e.target.value)} style={{width:160}}/></>}
        </div>
      </div>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {MONTHS.map((m,i)=>(
                <div key={m} style={{textAlign:"center",minWidth:62}}>
                  ...
                  <Inp type="number" value={splits[i]} onChange={e=>upSplit(i,e.target.value)} style={{textAlign:"center",padding:"4px",fontSize:12}} min={0} max={30}/>
      ...
      {tab==="detail"&&(
        <>
          <G2>
            ...
            <FRow label="Total Contract Value (THB)" tip="Recalculates all installments"><Inp type="number" value={f.totalContractValue} onChange={e=>setCv(e.target.value)}/></FRow>
...
const TaskRow = React.memo(({t,onSet,onDel,months}) => {
  ...
  // sync outward on blur only, keeping local state for typing
  const commitName = () => { if(!committed.current){onSet(t.id,"taskName",name);committed.current=false;} };
  // when parent updates (e.g. after add/del) sync name in
  ...
  return (
    <tr style={{borderBottom:"1px solid #f8fafc"}}>
      ...
      <td><Btn variant="danger" style={{fontSize:10,padding:"1px 5px"}} onClick={()=>onDel(t.id)}>×</Btn></td>
    </tr>
  );
});
...
const TaskTableWidget = ({tasks,onSet,onAdd,onDel,months}) => (
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,tableLayout:"fixed"}}>
      <colgroup><col style={{width:"44%"}}/><col style={{width:"10%"}}/><col style={{width:"10%"}}/><col style={{width:"10%"}}/><col style={{width:"12%"}}/><col style={{width:"10%"}}/><col style={{width:"4%"}}/></colgroup>
      <thead><tr style={{background:"#f8fafc"}}>
        {["Task / Activity","Mgr (days)","Sr (days)","Jr (days)","Total Cost","Pay Month",""].map(h=><th key={h} style={{padding:"5px 6px",textAlign:"left",fontWeight:700,color:"#64748b",fontSize:9,borderBottom:"1px solid #e2e8f0"}}>{h}</th>)}
      </tr></thead>
      <tbody>
...
const CostSheetPage = ({costSheets,onSave,customers,opps,user,onSaveOpp,toast,initSvcCode,onSvcReady}) => {
  ...
  const addTK=()=>sECS(p=>({...p,tasks:[...(p.tasks||[]),{id:uid(),taskName:"",manager:0,senior:0,junior:0,payMonth:1}]}));
  ...
  const addQTK=qid=>updQO(qid,q=>({...q,tasks:[...(q.tasks||[]),{id:uid(),taskName:"",manager:0,senior:0,junior:0,payMonth:1}]}));
          ...
          {(editCS.quoteOverrides||[]).map((q)=>{
            ...
            return (
              <Card key={q.id} style={{marginBottom:14,overflow:"hidden"}}>
                ...
                {/* Cost + margin footer */}
                <div style={{padding:"10px 20px",background:"#f8fafc",borderTop:"1px solid #e2e8f0",display:"flex",gap:14,flexWrap:"wrap",alignItems:"center"}}>
                  {[{l:"Internal COGS",v:qIC},{l:"External COGS (Co.)",v:qEC},{l:"OPEX",v:qOPEX},{l:"TOTAL COST",v:qTC,bold:true}].map(x=>(
                  ...
                  <div style={{flex:1}}/>
                  <div style={{padding:"9px 16px",borderRadius:7,background:+qMg>=30?"#dcfce7":"#fee2e2",textAlign:"center"}}>
                    <Span s={10} c={+qMg>=30?"#16a34a":"#dc2626"} style={{display:"block"}}>Actual Margin</Span>
                    <Span s={20} w={900} c={+qMg>=30?"#16a34a":"#dc2626"}>{qMg}%</Span>
